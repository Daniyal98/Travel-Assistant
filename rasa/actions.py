        ...
        if message == "in " + loc or message == "In " + loc or message == "IN " + loc:
            ...
            if event == "In which city?": 
                if loc in cities:
                    ...
                    api_key = "cbb96d08805df121ed620ddbe58f7d75"
...
class ActionBestHotels(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        loc = tracker.get_slot('city')
        filepath = "./scrapped_data/hotel_information/" + loc.lower() + "/Hotels.csv"
        ...
        buttons = [{"title": "Price of the first hotel?", "payload": '/inform_price{"position":"first"}'}, {"title": "Price of the second hotel?", "payload": '/inform_price{"position":"second"}'}, {"title": "Price of the third hotel?", "payload": '/inform_price{"position":"third"}'}]
...
class ActionPrice(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        for position in position_dictionary:
            if position in tracker.latest_message['text']:
                pos_exists = True
        ...
        city = ""
        if message1[3] in os.listdir('./scrapped_data/hotel_information'):
            city = message1[3]

        else:
            city = message1[4]

        if pos in list(position_dictionary.keys()) and pos_exists:
            filepath = "./scrapped_data/hotel_information/" + city.lower() + "/Hotels.csv"
            message2 = ""
            hotel = ""
            try:
...
class ActionPicture(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        if pos in list(position_dictionary.keys()) and pos_exists:
            ...
            dispatcher.utter_message(template="utter_picture", image_path="./scrapped_data/hotel_images/" + name.lower() + ".jpg")
...
class ActionPictureSpots(Action):
    def name(self):
        return "action_picture_spots"

    def run(self, dispatcher, tracker, domain):
        spot = tracker.get_slot('spot')

        dispatcher.utter_message(template="utter_picture", image_path="./scrapped_data/hotel_images/" + spot.lower() + ".jpg")
